//go:build ignore
// +build ignore

package main

import (
	"log"
	"time"

	"github.com/asciifaceman/tooey"
	"github.com/asciifaceman/tooey/themes"
	"github.com/asciifaceman/tooey/widgets"
)

/*
A simple text object demonstration that inherits element

TODO: events to exit
*/

func main() {
	if err := tooey.Init(); err != nil {
		log.Fatalf("failed to initialize tooey: %v", err)
	}
	defer tooey.Close()

	x, y := tooey.DrawableDimensions()

	hello := widgets.NewText()
	hello.SetTheme(themes.ThemeRetroTerminalOrange)

	hello.SetRect(0, 0, x, y)
	hello.Title.Content = "Element Title"

	hello.Content = "Some text to display within the element like a paragraph"

	tooey.Render(hello)

	time.Sleep(time.Duration(time.Second * 5))
}
